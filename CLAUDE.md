...
In Kubernetes, tokens live in `Secret/image-saver-tokens` (mounted as `/app/config/tokens.txt`). When the Secret is updated, K8s propagates the change to pod volume mounts within ~1 minute; the background reload task picks it up automatically — no rolling restart needed.

`/admin/reload-tokens` triggers an immediate reload on the pod that receives the request. For full fleet reload, rely on the background task or do a rolling restart.
